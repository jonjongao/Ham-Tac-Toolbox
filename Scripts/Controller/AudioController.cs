...
{
    static AudioController m_current;
    public static AudioController current
    {
        get
        {
            if (m_current == null)
            {
                var i = FindObjectOfType<AudioController>();
                m_current = i;
            }
            return m_current;
        }
    ...
    Dictionary<string, float> m_oneShotHistory = new Dictionary<string, float>();

    [SerializeField] string m_playingBGM;
    ...
    {
        var channel = AudioController.GetChannel(AudioChannel.BGM);
    ...
    public static void SetBGM(bool isOn, string clipID = null)
    {
        if (isOn)
        {
            ...
            if (string.IsNullOrEmpty(clipID) == false)
            {
                ...
                if (clip != null)
                {
                    AudioController.PlayClip(AudioChannel.BGM, clip, true);
        ...
        {
            AudioController.Stop(AudioChannel.BGM);
        }
    }
    ...
    public static void PlaySFX(string clipID)
    {
        ...
        if (clip != null)
        {
            AudioController.PlayClip(AudioChannel.SFX, clip);
        }
    }
    ...
    public static void SetVolume(AudioChannel target, float value)
    {
        ...
        switch (target)
        {
            case AudioChannel.BGM:
                m_current.m_masterMixer.SetFloat("BGMVol", vol);
                break;
            case AudioChannel.SFX:
                m_current.m_masterMixer.SetFloat("SFXVol", vol);
                break;
            default:
                m_current.m_masterMixer.SetFloat("MasterVol", value <= 0.01f ? -80f : vol);
                break;
        }
    }
